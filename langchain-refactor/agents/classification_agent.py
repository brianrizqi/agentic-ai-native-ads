    ...
    def classify(
        ...
        try:
            if self.provider == "local" and self.tokenizer:
                ...
                # Stage 95: Strict Retrieval
                # Increasing threshold to 0.45 to ensure higher retrieval purity.
...
JAWABAN: """
                    ...
                    # Stage 95: The 89% Calibrator
                    # Sticking with the high-performance minimalist prompt.
                ...
                if self.model_tier in ['micro', 'small'] and self.local_model_ref is not None:
                    with torch.no_grad():
                        ...
                        # Stage 95: Similarity-Weighted Voting
                        ...
                        # Stage 95: Aggressive Weighted Bias Calculation
                        # Global Ads Brake (Penalty) - Setting back to negative to counter excessive internal logit
                        base_bonus_ads = -1.2 if self.model_tier == 'small' else 4.38
                        ...
                        print(f"DEBUG [Stage 95] PPL: %.4f | {reason_msg}" % ppl_val)
                else:
                    with torch.no_grad():
