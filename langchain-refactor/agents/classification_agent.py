...
JAWABAN: """
                    ...
                    # Stage 96: Precision Linear Scaling
                ...
                if self.model_tier in ['micro', 'small'] and self.local_model_ref is not None:
                    with torch.no_grad():
                        ...
                        # Stage 96: Similarity-Weighted Voting
                        ...
                        # Stage 96: Precision Linear Bias Calculation
                        # Reverting global penalty to 0.0 (Neutral)
                        base_bonus_ads = 0.0 if self.model_tier == 'small' else 4.38
                        
                        # Apply smooth linear scaling based on weighted RAG evidence
                        rag_bias_news = min(4.5, rag_weighted_news * 2.2)
                        rag_bias_ads = min(2.0, rag_weighted_ads * 0.8)
                        ...
                        print(f"DEBUG [Stage 96] PPL: %.4f | {reason_msg}" % ppl_val)
                else:
                    with torch.no_grad():
