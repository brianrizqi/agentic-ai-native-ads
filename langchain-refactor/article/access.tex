...
\begin{enumerate}
    ...
    \item \textbf{Methodological contribution}: We introduce a ``Reasoning-First'' fine-tuning strategy using RAG with 12,088 Indonesian news articles to reduce hallucination and improve detection accuracy.
...
\label{sec:dataset}
The foundation of this study is an extensive dataset of Indonesian-language electronic news articles, specifically curated for native advertisement detection \cite{b17}. The dataset comprises 12,088 articles collected from six major Indonesian news portals (e.g., Kompas, Detik, Tempo) between February and August 2022. While the original corpus was published as a structured benchmark in \textit{Scientific Data} \cite{b17}, it was further adapted and reformatted in this work to support instruction-based fine-tuning for SLM/LLM models, including the conversion of multi-label annotations into Reasoning-First prompt-completion pairs. This adapted version is publicly available as a dedicated LLM/SLM dataset on figshare \cite{b68}. This collection covers diverse categories as summarized in Table \ref{tab:dataset_stats}.
...
\begin{enumerate}
    \item \textbf{Text Normalization}: Regex normalization is used to detect the semantic hierarchy of the article, removing spatial redundancy phrases and dangling URLs.
    \item \textbf{Feature Extraction}: Compiles syntactic lexicon libraries and NumPy feature extraction for text length density variables, unique phrase percentages, and capitalization ratio coefficients in a programmatically statistical manner without deep NLP inference.
\end{enumerate}
...
\caption{Comparison of the four backbone architectures used in the AGENA experiments: Gemma 3 12B, Qwen 14B, Llama 3.1 8B, and GPT-OSS 20B.}
\label{fig:model_architectures}
\end{figure}

As shown in Figure~\ref{fig:model_architectures}, Gemma 3 12B is the primary production backbone in AGENA because it offers strong reasoning quality under parameter-efficient adaptation and stable behavior on Non-Promotional Brand Reporting cases. In the same figure, Qwen 14B provides high representational capacity and competitive instruction-following performance, but in our setting is more prone to over-triggering promotional cues in ambiguous news.

Additionally, Llama 3.1 8B was selected as a compact yet capable baseline with lower computational demand and predictable fine-tuning behavior. Finally, GPT-OSS 20B was included to examine a larger open-weight architecture under constrained adaptation schedules. This cross-architecture design allows us to compare not only accuracy, but also reasoning consistency, calibration in borderline samples, and practical deployment trade-offs between model size and interpretability quality.

Most models share a common LoRA adapter configuration of rank $r=16$ and alpha $\alpha=32$, trained at a learning rate of $2\times10^{-5}$ with an effective batch size of 8 (1 per-device batch accumulated over 8 gradient steps). Notable exceptions are Gemma 3 (270M), which uses a substantially larger adapter ($r=64$, $\alpha=128$), a higher learning rate ($10^{-4}$), and the largest effective batch (16) to compensate for its smaller capacity; Gemma 2 (9B) and Llama 3.1 (8B), which use an effective batch of 4 for more frequent optimizer updates; and GPT-OSS (20B), which applies a reduced adapter capacity ($r=8$, $\alpha=16$) combined with an elevated learning rate ($8\times10^{-5}$) to accommodate its constrained adaptation schedule. All models were trained using 4-bit quantization (QLoRA) via the Unsloth framework with FlashAttention-2 to reduce VRAM footprint and accelerate convergence.
...
\begin{enumerate}
    ...
    \item \textbf{Non-Promotional Brand Reporting Check}: The ability to accurately classify editorial news that extensively mentions corporate entities without inherently promoting them.
\end{enumerate}
...
The experiments were conducted using the Indonesian news dataset comprising 12,088 articles. The LLM components (Classifier and Explainer) were fine-tuned using the Unsloth framework with LoRA adapters (Rank=16, Alpha=32). Training was performed on a high-performance computing infrastructure. The dataset was split into training and test sets, with the test set encompassing a diverse range of Non-Promotional Brand Reporting cases to evaluate model robustness.
...
\begin{table*}[t]
\caption{Consolidated Results: Fine-Tuned LLM Variants, Ablation Study, and SENADA Baseline Comparison}
\label{tab:results_consolidated}
\begin{center}
\begin{tabular}{|l|c|c|c|c|}
\hline
\textbf{Model / Configuration} & \textbf{Recall} & \textbf{Precision} & \textbf{F1-score} & \textbf{Accuracy} \\
\hline
\multicolumn{5}{|l|}{\textit{(A) Fine-Tuned LLM Variants (Full AGENA Pipeline)}} \\
\hline
...
\hline
\multicolumn{5}{|l|}{\textit{(B) Ablation Study --- Gemma 3 (12B) Backbone}} \\
\hline
\multicolumn{3}{|l|}{Baseline (Zero-Shot)} & 0.762 & 78.40\% \\
\multicolumn{3}{|l|}{Classifier + Preprocessing only} & 0.831 & 84.60\% \\
\multicolumn{3}{|l|}{Classifier + RAG (no Preprocessing)} & 0.898 & 91.20\% \\
\multicolumn{3}{|l|}{\textbf{Full AGENA Pipeline}} & \textbf{0.943} & \textbf{94.50\%} \\
\hline
\multicolumn{5}{|l|}{\textit{(C) Comparison with Prior State-of-the-Art}} \\
\hline
\multicolumn{3}{|l|}{SENADA \cite{b19} (Stacked Ensemble)} & 0.927 & 93.00\% \\
...
Table~\ref{tab:results_consolidated} consolidates three evaluation perspectives into a single reference: the cross-model comparison (Group A), the ablation study (Group B), and the comparison against the prior state-of-the-art SENADA baseline (Group C).

\textbf{Model-level analysis (Group A).} Results from the ``Reasoning-First'' prompt alignment reveal a clear performance gradient following model scale and instruction-following capability. Gemma 3 (12B) achieves the highest overall accuracy at 94.50\%, with a precision of 0.938 on the Native Ads class. This model handles the most challenging cases effectively --- most notably, it correctly identifies editorial content that mentions corporate entities without promotional framing (Non-Promotional Brand Reporting), a task where several larger-parameter models fail. Gemma 3 (4B) achieves 92.00\%, narrowly outperforming the older Gemma 2 (9B) at 90.50\%, confirming that architectural advancements in the latest Gemma generation contribute meaningfully to domain-specific instruction-following quality, even at a smaller parameter count.

As model size decreases below 1B parameters, reliable reasoning becomes difficult to maintain. Gemma 3 (1B) dropped to 76.50\%, exhibiting very high recall (0.979) but low precision (0.676), indicating a systematic tendency to over-flag brand mentions as promotional regardless of editorial context. The 270M model scored 52.00\% and frequently produced structurally invalid or incomplete outputs, confirming that the Reasoning-First JSON format is incompatible with sub-500M parameter architectures. Qwen 2.5 14B (71.50\%) and Llama 3.1 8B (61.50\%) underperformed relative to their parameter counts, likely because their pretraining corpora did not include sufficient Indonesian journalistic data to support fine-grained discrimination between commercial and editorial framing. GPT-OSS (57.50\%) exhibited similar behavior, defaulting to surface-level keyword matching rather than contextual reasoning. These results confirm the design choice of centering AGENA's Classifier Agent on Gemma 3 12B, and demonstrate that the pipeline architecture itself --- not the backbone model parameter count alone --- drives the system's top-tier performance.

\textbf{Ablation study (Group B).} The ablation study progressively adds pipeline components to isolate the individual contribution of each agent, using the Gemma 3 12B backbone throughout all configurations.

\textit{(1) Baseline (Zero-Shot), Accuracy: 78.4\%, F1: 0.762.} This is the unaugmented Gemma 3 12B model applied directly to the article without any preprocessing, retrieval, or structured prompting. It represents the performance ceiling achievable by raw in-context generalization alone. At 78.4\%, the model already demonstrates reasonable domain awareness, but makes frequent errors on Non-Promotional Brand Reporting cases where brand mentions appear in purely factual reporting contexts, as it has no external reference to distinguish editorial tone from promotional framing.

\textit{(2) Classifier + Preprocessing only, Accuracy: 84.6\%, F1: 0.831.} Adding the Preprocessing Agent --- which extracts structured linguistic features such as lexical density, capitalization ratio, and persuasive cue counts --- yields a +6.2 point accuracy gain. This confirms that explicit, rule-based feature signals provide meaningful guidance to the Classifier Agent, helping it prioritize statistical indicators of promotional intent and reducing sensitivity to superficial keyword triggers that caused errors in the zero-shot baseline.

\textit{(3) Classifier + RAG (no Preprocessing), Accuracy: 91.2\%, F1: 0.898.} Replacing preprocessing with the Retriever Agent and its FAISS-backed knowledge base produces the single largest gain in the study: +12.8 points over the zero-shot baseline and +6.6 points over the preprocessing-only variant. The Retriever provides the Classifier with the five most semantically similar historical articles and their ground-truth labels from the 12,088-article knowledge base. This in-context evidence from verified examples is evidently far more discriminative than hand-crafted feature signals, particularly for Non-Promotional Brand Reporting cases where subtle differences in narrative framing --- not surface keywords --- determine the correct label.

\textit{(4) Full AGENA Pipeline, Accuracy: 94.5\%, F1: 0.943.} The full five-agent system, combining Preprocessing, RAG, Classifier, and Explainer, achieves a further +3.3 point gain over the RAG-only variant. The compounding effect reflects two mechanisms: first, the Preprocessing Agent stabilizes the Classifier's input representation across heterogeneous portal formats, reducing variance in extracted article text; second, the Explainer Agent's structured JSON output format enforces consistency in the Classifier step, reducing response parsing failures that can propagate as incorrect labels. Together, these factors push accuracy to 94.5\% and confirm that each pipeline stage provides an independently justified contribution to the final result.

\textbf{SENADA comparison (Group C).} AGENA surpasses SENADA \cite{b19}, the prior state-of-the-art ensemble model on the same Indonesian native advertisement dataset, achieving a +1.5 percentage point improvement in accuracy (93.0\% $\to$ 94.5\%) and a +0.016 F1-score gain. The precision improvement is notably larger: SENADA's precision of 0.778 rises to 0.938 in AGENA, a +16.0 point gain that reflects AGENA's stronger ability to minimize false positives --- critical in editorial settings where incorrectly flagging a legitimate news article carries professional consequences. SENADA achieves this accuracy by decomposing the task into four parallel ensemble sub-models (Sentiment, Persuasion, Promotion, Perspective), each trained on static feature sets, and aggregating their independent label votes. While effective, this design cannot access dynamic, inference-time contextual evidence, produces no per-article reasoning traces, and cannot explain its decisions to human editors. AGENA replaces this with a sequential agentic workflow in which the Retriever Agent provides real-time context from the 12,088-article knowledge base and the Explainer Agent produces structured, human-verifiable justifications for each classification decision. This interpretability layer is the primary qualitative advancement over SENADA and is essential for adoption in editorial workflows where human oversight is required \cite{b20}.
...
\subsection{Analysis of Non-Promotional Brand Reporting Cases}
A consistent source of misclassification across all tested models was the Non-Promotional Brand Reporting category. These are factually accurate news articles that mention corporate entities, products, or government programs without promotional intent. For example, infrastructure reporting routinely names private contractors, and AGENA's Classifier Agent occasionally flagged these based on keyword patterns rather than narrative context. This ``brand-trigger'' bias was more pronounced in Qwen and Llama models than in Gemma 3 12B. Addressing this in future work requires expanding the knowledge base with a larger proportion of Non-Promotional Brand Reporting examples to improve negative discrimination.
...
This paper presented AGENA, a multi-agent AI framework for explainable native advertisement detection in Indonesian digital news. The framework contributes toward addressing two key limitations of prior work: the lack of interpretability in ensemble-based classifiers, and the tendency of generic LLMs to hallucinate when applied to domain-specific Indonesian journalism. By combining a RAG-grounded knowledge base with a Reasoning-First fine-tuning strategy, AGENA achieves 94.5\% classification accuracy using Gemma 3 12B as the Classifier Agent within our experimental setup and produces structured reasoning traces that can be reviewed editorially. Notably, this framework outperforms the previous non-agentic baseline, SENADA, gaining +4.5\% in overall accuracy and +16.0\% in Native Ad precision \cite{b19}.

The ablation study confirms that each pipeline component contributes to overall performance, with the Retriever Agent providing the largest single gain (+12.8\%). The LLM-as-a-Judge evaluation protocol further demonstrates that accuracy alone is an insufficient measure: smaller models that achieve borderline accuracy do so through shallow or hallucinated reasoning rather than genuine linguistic understanding. Future work will focus on extending the knowledge base to include more Non-Promotional Brand Reporting examples from regional Indonesian news portals, reducing per-article latency for real-time applications, and evaluating AGENA on cross-lingual news environments. In addition, stronger transformer baselines and parameter-efficient adaptation schemes remain important for practical deployment. This deployment direction is also compatible with efficient engineering practices shown in prior local framework work.
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{brian.jpg}}]{Brian Rizqi Paradisiaca Darnoto} received the doctor's degree in informatics from the Institut Teknologi Sepuluh Nopember, Surabaya, Indonesia, in 2025. He is currently a Postdoctoral Researcher at the Institut Teknologi Sepuluh Nopember and a Lecturer at the University of Jember, Indonesia. His research interests include artificial intelligence, natural language processing, and software engineering.
\end{IEEEbiography}
