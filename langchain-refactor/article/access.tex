...
\textbf{Model-level analysis (Group A).} Results from the ``Reasoning-First'' prompt alignment reveal a clear performance gradient following model scale and instruction-following capability. Gemma 3 (12B) achieves the highest overall accuracy at 94.50\%, with a precision of 0.938 on the Native Ads class. This model handles the most challenging cases effectively --- most notably, it correctly identifies editorial content that mentions corporate entities without promotional framing (Borderline News), a task where several larger-parameter models fail. Gemma 3 (4B) achieves 92.00\%, narrowly outperforming the older Gemma 2 (9B) at 90.50\%, confirming that architectural advancements in the latest Gemma generation contribute meaningfully to domain-specific instruction-following quality, even at a smaller parameter count.
